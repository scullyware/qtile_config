...
def autostart():
    home = os.path.expanduser('/home/gleb/.config/qtile/autostart.sh')
...
groups = [
    Group('WEB |1|', matches=[Match(wm_class=['Firefox-esr'])]),
    Group('TERM |2|', matches=[Match(wm_class=["lxterminal", "Lxterminal", "xfce4-terminal", "Xfce4-terminal", "Терминал - gleb@glebtop: ~", "gleb@glebtop: ~"])]),
    Group('TEXT |3|', matches=[Match(wm_class=['Bluefish'])]),
    Group('CHAT |4|', matches=[Match(wm_class=["Telegram", "TelegramDesktop", "skype", "Skype"])]),
    Group('FILE |5|', matches=[Match(wm_class=['pcmanfm'])]),
    Group('MEDIA |6|', matches=[Match(wm_class=['vlc'])]),
    Group('VNC |7|', matches=[Match(wm_class=['vinagre'])]),
    Group('OTHER |8|'),
]
...
def init_widgets():
    ...
    widgets = [
        ...
        GroupBox(fontsize=11, padding=2.8, borderwidth=1, urgent_border=DARK_BLUE,
...
def init_top_bar():
    return bar.Bar(widgets=init_widgets(), size=24, opacity=0.8)
